...
import RouteTripModel, { RouteTripDocument } from '../../database/models/trip.model';
import { TripStatusEnum } from '../../common/enums/trip-status.enum';

export class TripService {
  public async getDirections(origin: string, destination: string, vehicle: string = 'car') {
    try {
  ...
  public async createTrip(data: CreateTripData): Promise<RouteTripDocument> {
    const { userId, origin, destination, startTime, endTime, availableSeats, notes } = data;
    ...
    try {
        ...
        if (!originData?.geometry?.location || !destinationData?.geometry?.location) {
        ...
        if (error.name === 'ValidationError') {
            throw new NotFoundException(`Failed to create trip: ${error.message}`);
        }
        throw new NotFoundException(`Failed to create trip: ${error.message || 'Unknown error'}`);
    }
  }
  ...
  private async generateTripCode(): Promise<string> {
    ...
    while (attempts < maxAttempts) {
        ...
        for (let i = 0; i < length; i++) {
            tripCode += characters.charAt(Math.floor(Math.random() * characters.length));
        }


        const existingTrip = await RouteTripModel.findOne({ tripCode, isDeleted: false });
        if (!existingTrip) {
            return tripCode;
        }
    ...
    throw new NotFoundException('Unable to generate unique tripCode after multiple attempts');
  }
  
  public async searchTrips(searchParams: SearchTripParams): Promise<RouteTripDocument[]> {
    ...
    try {
      ...
      const query: any = {
        'startLocation.address': { $regex: originData.formatted_address, $options: 'i' },
        'endLocation.address': { $regex: destinationData.formatted_address, $options: 'i' },
        'schedule.startTime': {
          $gte: startDate,
          $lte: endDate,
        },
      ...
      if (trips.length === 0) {
        throw new NotFoundException('No trips found matching your criteria');
      }
